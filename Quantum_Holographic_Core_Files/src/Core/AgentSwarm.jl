...
function spawn_agent!(mgr::AgentSwarmManager, name::String, role::AgentRole; caps::Vector{String}=String[])
    ...
    println("ðŸš€ Agent Spawned: [$(agent.name)] Role: $(agent.role)")
    agent.id
end
...
function dispatch_task!(mgr::AgentSwarmManager, agent_id::UUID, task::String)
    if haskey(mgr.agents, agent_id)
        ...
        println("ðŸ“¡ Dispatching to [$(agent.name)]: $task")
        return true
    end
    false
end
...
function list_swarm(mgr::AgentSwarmManager)
    println("\nðŸ“Š Agent Swarm [$(mgr.swarm_name)]")
    println("-" ^ 40)
    for agent in values(mgr.agents)
        status_icon = (agent.status == IDLE) ? "ðŸŸ¢" : (agent.status == BUSY) ? "ðŸŸ¡" : "ðŸ”´"
...
function run_swarm_loop!(mgr::AgentSwarmManager; cycles::Int=100)
    println("ðŸŒ€ Initiating Sovereign Code Swarm Loop... [Cycles: $cycles]")
    ...
    println("ðŸŒ€ Swarm Loop Concluded.")
end
...
function demo()
    println("ðŸŒŒ AgentSwarm.jl Demo")
    println("=" ^ 50)
