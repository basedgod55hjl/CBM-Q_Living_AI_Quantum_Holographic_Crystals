...
function plan_path(planner::NavigationPlanner, start::Vector{Float64}, 
    ...
    println("ðŸ—ºï¸ Planning Path: $(start) â†’ $(goal)")
...
function demo()
    println("ðŸ¤– Robotics Bindings Demo")
    println("=" ^ 50)
    ...
    for waypoint in path[1:min(3, length(path))]
        ...
        println("ðŸ“ Robot Position: $(robot.position)")
    end
